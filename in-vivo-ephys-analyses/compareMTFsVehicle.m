...
for q = 1:numel(control)
    ...
    % analyze the data
    [spikeCounts synchrony_vehicle phase] = analyzeAMfromStructNMDAproj(dynamicFields, 0, 0, [1:6]);
...
for q = 1:numel(control)
    ...
    % plot drug condition
    subplot(3, 4, q);
    plot(log2(freqs), synchrony_vehicle_list(q, :), 'Color', '#2a9d8f');
    hold on
...
% Add labels and title
sgtitle('Temporal MTFs from Control and vehicle');
